...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    async fetchUnread() {
      try { const { data } = await axios.get('/chat/unread-count'); this.unread = data.unread || 0; }
      catch (e) { /* silencioso */ }
    },
    async fetchConversations() {
      try { const { data } = await axios.get('/chat/conversations'); this.conversations = data || []; }
      catch (e) { /* silencioso */ }
    },
    async fetchUsers() {
      try { const { data } = await axios.get('/chat/users'); this.users = data || []; }
      catch (e) { /* silencioso */ }
    },
    ...
    async openConversation(convId) {
      ...
      } catch (e) { this.messages = []; this.other = null; }
      finally { this.loadingMsgs = false; }
    },
    // Refresco ligero mientras la conversación está abierta (mensajes entrantes + lecturas).
    async refreshActive() {
    ...
    async markRead(convId) {
      try { await axios.post(`/chat/conversations/${convId}/read`); this.fetchUnread(); }
      catch (e) { /* silencioso */ }
    },
    clearActive() { this.activeId = null; this.messages = []; this.other = null; },
  },
});
